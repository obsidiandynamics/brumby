...
cargo build -p brumby
cargo build -p brumby-racing
cargo build -p brumby-soccer

echo "Testing $app_name"
export LLVM_PROFILE_FILE="${app_name}-%p-%m.profraw"
cargo test -p brumby --tests # don't run doctests
cargo test -p brumby-racing --tests # don't run doctests
cargo test -p brumby-soccer --tests # don't run doctests
